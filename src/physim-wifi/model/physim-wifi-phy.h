...
private:
  ...
  void NotifyRxCollision (Ptr<const Packet> packet, Ptr<const PhySimWifiPhyTag> tag, Ptr<const PhySimInterferenceHelper::Event> event);
...
//  static bool m_rngReset;
  ...
  /*
   * Trace to signal that a packet was dropped (capture failed) possibly due to collision.
   * The first parameter of the callback is the interference event corresponding to the dropped packet.
   * The second parameter of the callback is the list of interference events at the time of the packet loss.
   *  (Note that not all packets on this list actually overlap the dropped packet,
   *   so a trace callback should call PhySimInterferenceHelper::Event::Overlaps to
   *   verify this.)
   */
  TracedCallback<Ptr<const PhySimInterferenceHelper::Event>, const std::list<Ptr<PhySimInterferenceHelper::Event> >& > m_collisionTrace;
