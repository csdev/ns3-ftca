...
public:
  ...
  {
public:
    Event (Ptr<const Packet> packet, Ptr<PhySimWifiPhyTag> tag);
    virtual ~Event ();
    ...
    /**
     * Returns whether this event overlaps with another event.
     * \param time The interference event against which to check the overlap
     */
    bool Overlaps (Ptr<const PhySimInterferenceHelper::Event> other) const;
    /**
    ...
    /**
     * Returns the underlying Packet associated with this interference event. This is used for tracing only.
     */
...
private:
  ...
  /**
   * \return the list of interference events, used for tracing
   */
  const std::list< Ptr<PhySimInterferenceHelper::Event> >& GetEvents (void) const;
