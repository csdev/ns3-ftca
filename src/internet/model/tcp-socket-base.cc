...
//                   MakeEnumChecker (CLOSED, "Closed"))
    ...
    .AddAttribute ("AckPriority", "Set high QoS priority for TCP control packets",
                   BooleanValue (false),
                   MakeBooleanAccessor (&TcpSocketBase::m_ackPriority),
                   MakeBooleanChecker ())
    .AddTraceSource ("RTO",
...
TcpSocketBase::TcpSocketBase (const TcpSocketBase& sock)
  : TcpSocket (sock), //copy object::m_tid and socket::callbacks
    ...
    m_rWnd (sock.m_rWnd),
    m_ackPriority (sock.m_ackPriority)
{
  NS_LOG_FUNCTION (this);
...
TcpSocketBase::SendEmptyPacket (uint8_t flags)
{
  ...
  if (isAck)
    {
      DpiTag ackTag (DpiTag::TCP_ACK);
      p->AddPacketTag (ackTag);
    }

  if (m_ackPriority && (hasSyn || isAck))
    {
      QosTag qosTag;
      qosTag.SetTid (7);
