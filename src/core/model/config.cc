...
MatchContainer::Connect (std::string name, const CallbackBase &cb)
{
  ...
  if (m_objects.size () == 0)
    NS_FATAL_ERROR ("Failed to connect: No matching objects for trace path");
  bool isConnected = false;
  for (uint32_t i = 0; i < m_objects.size (); ++i)
    {
      Ptr<Object> object = m_objects[i];
      std::string ctx = m_contexts[i] + name;
      bool isOneConnected = object->TraceConnect (name, ctx, cb);
      if (isOneConnected)
        isConnected = true;
    }
  if (!isConnected)
    NS_FATAL_ERROR ("Failed to connect: No matching trace source for attribute " << name);
}
void 
MatchContainer::ConnectWithoutContext (std::string name, const CallbackBase &cb)
{
  ...
  for (Iterator tmp = Begin (); tmp != End (); ++tmp)
    {
      Ptr<Object> object = *tmp;
      bool isOneConnected = object->TraceConnectWithoutContext (name, cb);
      if (isOneConnected)
        isConnected = true;
    }
  if (!isConnected)
...
Resolver::DoArrayResolve (std::string path, const ObjectPtrContainerValue &vector)
{
  ...
  if (vector.GetN () == 0)
    NS_LOG_WARN ("Warning: calling DoArrayResolve on empty vector -- no matches will be returned");
