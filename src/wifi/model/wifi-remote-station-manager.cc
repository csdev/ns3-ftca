...
WifiRemoteStationManager::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::WifiRemoteStationManager")
    ...
    .AddAttribute ("NonUnicastMode", "Wifi mode used for non-unicast transmissions.",
                   ...
                   MakeWifiModeChecker ())
    .AddAttribute ("DpiForTcpAckRate", "Perform deep packet inspection and transmit TCP ACK at the MAC control packet rate.",
                   BooleanValue (false),
                   MakeBooleanAccessor (&WifiRemoteStationManager::m_enableDpi),
                   MakeBooleanChecker ())
...
WifiRemoteStationManager::GetDataMode (Mac48Address address, const WifiMacHeader *header,
                                       Ptr<const Packet> packet, uint32_t fullPacketSize)
{
  if (m_enableDpi && DpiTag::GetPacketType (packet) == DpiTag::TCP_ACK)
    return GetRtsMode (address, header, packet);
